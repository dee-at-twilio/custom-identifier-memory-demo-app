...
async def _ai_agent_profile_id() -> str:
    ...
    # The AI profile is the only one that carries the `phone` identifier.
...
async def get_state(viewedProfileId: str | None = None) -> dict:
    ...
    profiles_raw = await tw.list_profiles(STORE_ID, known_ids)
...
async def assign_job(body: AssignJobBody) -> dict:
    ...
    profile_id = prof["id"]
    # Record the phone -> profile mapping locally so inbound-SMS routing can
    # find this profile later (Memory has no `mobile`/`phone` identifier for techs).
    state.record_profile(TECH_PHONE, profile_id)
...
async def delete_profile(profile_id: str) -> dict:
    ...
    # Drop the local phone-index entry so the profile stops showing up in
    # /api/state and doesn't get scanned by inbound-SMS routing.
    mobile = _job_traits(prof).get("mobile") or TECH_PHONE
    state.forget_profile(mobile, profile_id)
...
async def reactivate_job(body: ProfileBody) -> dict:
    ...
    # Belt-and-suspenders: reactivating a profile the app already knows about
    # should be a no-op for the index, but re-record in case state.json was
    # wiped or the profile was created out-of-band.
    state.record_profile(TECH_PHONE, body.profileId)
